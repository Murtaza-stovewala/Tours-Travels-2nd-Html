...
document.addEventListener('DOMContentLoaded', () => {
  ...
  currentFlight = flightsData.find(f => Number(f.id) === flightId);
  if (!currentFlight) return showNotFound();
  ...
  if (form) {
    form.addEventListener('submit', (e) => {
      e.preventDefault();
      alert('Thanks! We will contact you to finalize booking.');
      closeBookingModal();
      form.reset();
    });
  }
...
function renderContent() {
  const mainSchedule = (currentFlight.schedule && currentFlight.schedule.length > 0) 
    ...
    : { airline: currentFlight.airline, depart: '07:40 AM', arrive: '08:47 AM', type: currentFlight.stops, price: currentFlight.price };
  ...
  c.innerHTML = `
    ...
    <div class="bg-white rounded-3xl shadow-lg border border-gray-100 overflow-hidden">
      <div class="bg-blue-50/50 p-4 border-b border-gray-100 flex items-center gap-3">
        <svg class="h-5 w-5 text-cyan-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 12h14M12 5l7 7-7 7"/></svg>
        <span class="font-semibold text-gray-700 text-sm">Flight from ${currentFlight.fromCode}, ${currentFlight.from} → ${currentFlight.toCode}, ${currentFlight.to}</span>
      </div>
        ...
        <div class="flex flex-col md:flex-row justify-between items-center border-t border-dashed border-gray-200 pt-6 gap-4">
          <div class="flex gap-4">
            <div class="flex items-center text-green-600 text-sm font-medium bg-green-50 px-3 py-1 rounded-full">
              <svg class="h-4 w-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
              Refundable
            </div>
    ...
    <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-6 md:p-8">
      ...
      <div class="space-y-4">
        ${['Booking hotel and flight together saves you up to 30%', 'Add airport transfer for special price for you!', 'Get travel insurance with 50% discount'].map(text => `
          <div class="flex items-center justify-between p-4 bg-white border border-gray-100 rounded-2xl hover:shadow-md transition-shadow cursor-pointer group">
            ...
            <svg class="h-5 w-5 text-gray-400 group-hover:text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
          </div>
        `).join('')}
      </div>
      <button class="w-full mt-6 rounded-full border-2 border-blue-200 text-blue-600 font-bold hover:bg-blue-50 py-3">Show more extra convenience for your trip</button>
    </div>
          ...
          <div class="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm flex flex-col gap-4">
            <div class="flex justify-between items-center">
              ...
              <div class="flex flex-col items-center px-2">
                ...
                <svg class="h-4 w-4 text-cyan-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 12h14M12 5l7 7-7 7"/></svg>
    ...
    <div class="bg-white rounded-3xl p-6 shadow-lg border border-gray-100">
      ...
      <div class="flex flex-col items-center mb-8 pb-8 border-b border-dashed border-gray-200">
        <div class="h-20 w-20 rounded-full bg-orange-100 mb-4 flex items-center justify-center">
          <svg class="h-10 w-10 text-orange-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
        </div>
      ...
      <button onclick="window.location.href='contact.html'" class="w-full bg-orange-500 hover:bg-orange-600 text-white font-bold py-6 rounded-xl shadow-lg transition-transform hover:scale-[1.02]">Continue Booking</button>
    </div>
  `;
}
